...
jQuery('.ApWait').show(); 
		    $.ajax({
		        url: '/hrportalv2/hr/employe/store-step1',
		        type: 'POST',
		        data: formData,
...
jQuery('.ApWait').show(); 
		    $.ajax({
		        url: '/hrportalv2/hr/employe/store-step2',
		        type: 'POST',
		        data: formData,
		        contentType: false,      // ✅ Do not set content type manually
		        processData: false,      // ✅ Prevent jQuery from converting the FormData
...
jQuery('.ApWait').show(); 
		    $.ajax({
		        url: '/hrportalv2/hr/employe/store-step3',
		        type: 'POST',
		        data: formData,
		        contentType: false,      // ✅ Do not set content type manually
		        processData: false,      // ✅ Prevent jQuery from converting the FormData
...
jQuery('.ApWait').show(); 
		    $.ajax({
		        url: '/hrportalv2/hr/employe/store-step4',
		        type: 'POST',
		        data: formData,
		        contentType: false,      // ✅ Do not set content type manually
		        processData: false,      // ✅ Prevent jQuery from converting the FormData
...
jQuery('.ApWait').show(); 
		    $.ajax({
		        url: '/hrportalv2/hr/employe/store-step5',
		        type: 'POST',
		        data: formData,
		        contentType: false,      // ✅ Do not set content type manually
		        processData: false,      // ✅ Prevent jQuery from converting the FormData
...
jQuery('.ApWait').show(); 
		    $.ajax({
		        url: '/hrportalv2/hr/employe/store-step6',
		        type: 'POST',
		        data: formData,
		        contentType: false,      // ✅ Do not set content type manually
		        processData: false,      // ✅ Prevent jQuery from converting the FormData
		        ...
		        success: function (response) {
		        	jQuery('.ApWait').hide(); 
		                   Swal.fire({
					            ...
					            window.location.href = "/hrportalv2/hr/dashboard";
					        });
		        },
		        error: function (xhr) {
		        	jQuery('.ApWait').hide(); 
...
window.addSkillField = function () {
    ...
    updateSkillDropdowns(); // if needed
        ...
        //     // Remove the whole skill-group div
        //     el.closest('.skill-group').remove();

        //     // Optional: remove existing error if any
        //     const errorSpan = container.querySelector('.error');
        //     if (errorSpan) {
        //         errorSpan.remove();
        //     }
        // } else {
        //     // Remove previous error if already shown
        //     container.querySelectorAll('.error').forEach(el => el.remove());

        //     // Show error after container
        //     const error = document.createElement('span');
        //     error.className = 'error text-danger';
        //     error.innerText = 'At least one skill is required.';
        ...
        document.addEventListener("DOMContentLoaded", function () {
            ...
            window.removeDocument = function (element) {
                ...
                //     group.remove();
                // } else {
                //     group.querySelectorAll('.error').forEach(el => el.remove());

                //     const error = document.createElement('span');
                //     error.className = 'error text-danger';
                //     error.innerText = 'At least one document is required.';
